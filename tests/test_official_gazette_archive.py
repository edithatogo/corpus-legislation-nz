...
def test_official_gazette_archive_cli_writes_bundle_and_manifest(tmp_path: Path) -> None:
    ...
    assert (
        output_dir / "corpus-legislation-nz-gazette-official-2026.validation_report.json"
    ).exists()
    assert (
        output_dir / "corpus-legislation-nz-gazette-official-2026.coverage_report.json"
    ).exists()
    assert (
